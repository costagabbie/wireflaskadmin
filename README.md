...
## Project status
It's in early development stage, the webapp is pretty much done, lacking only the API, some UI/UX refinements, and also some extensive testing not tested, and the daemon is implemented but need to be properly tested and possibly some work on exception handling.
